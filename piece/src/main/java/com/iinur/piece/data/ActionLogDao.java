...
public class ActionLogDao extends BaseDao {
	...
	public void insert(String url, String action_name, int user_id,
			...
			int product_chat_id, int tag_id) {
		StringBuffer sql = new StringBuffer();
		sql.append("INSERT INTO action_log ");
		sql.append("(url,action_name,user_id,project_id,piece_id,product_id,chat_id,product_chat_id,tag_id) ");
		sql.append("VALUES (?,?,?,?,?,?,?,?,?)");
		try {
			run.update(sql.toString(), url, action_name, user_id, project_id,
					piece_id, product_id, chat_id, product_chat_id, tag_id);
